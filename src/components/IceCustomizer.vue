  ...
  <div class="welcome-text" v-if="!orderConfirmed && step < 3">
    <div class="wood-sign">
      ...
      <div class="sign-content">
        <h1>
          Welkom bij <span class="brand">Ben & Jerry’s</span> ijsconfigurator
        </h1>
  ...
  <transition name="card-pop" appear>
    <div v-if="orderConfirmed" class="confirmation-card">
      ...
      <p class="confirmation-text">
        Bedankt voor je bestelling, <strong>{{ order.name }}</strong
        >! Je ijsje is onderweg.
      </p>
      <ul class="confirmation-details">
        ...
        <li>
          <span class="label">Totaal:</span> €{{ lastOrder.price.toFixed(2) }}
        </li>
      </ul>
...
const canCheckout = computed(
  () => order.name && order.address && totalPrice.value > 0 && !error.value
);
...
function handleCanvasClick(e) {
  ...
  isIceSelected.value =
    iceMesh && raycaster.intersectObject(iceMesh).length > 0;
}
